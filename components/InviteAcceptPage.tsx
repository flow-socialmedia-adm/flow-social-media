...
const InviteAcceptPage: React.FC = () => {
	...
	useEffect(() => {
		...
		(async () => {
			...
			try {
				...
				const prev = await apiGetPublic<{ valid: boolean; fullName?: string; email?: string }>(
	...
	const submit = async (e: React.FormEvent) => {
		...
		setError(null);
		try {
			await apiPostPublic('/auth/invite/accept', {
				token,
			...
			let msg = 'Não foi possível ativar o convite.';
			try {
	...
	if (!token || !valid) {
		return (
			<div className="flex min-h-screen flex-col items-center justify-center gap-4 bg-gray-50 p-6 text-center dark:bg-gray-900">
				<p className="text-gray-700 dark:text-gray-300">Convite inválido ou expirado.</p>
	...
	if (done) {
		return (
			<div className="flex min-h-screen flex-col items-center justify-center gap-4 bg-gray-50 p-6 dark:bg-gray-900">
				<p className="text-lg font-medium text-gray-900 dark:text-white">Conta ativada com sucesso.</p>
				<Link
					to="/"
	...
	return (
		<div className="flex min-h-screen flex-col items-center justify-center gap-6 bg-gray-50 p-6 dark:bg-gray-900">
			...
			>
				<h1 className="text-xl font-bold text-gray-900 dark:text-white">Aceitar convite</h1>
				<p className="text-sm text-gray-600 dark:text-gray-400">{email}</p>
				<div>
					<label htmlFor="invite-name" className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">
						Nome completo
					</label>
					<input
						id="invite-name"
				...
				<div>
					<label htmlFor="invite-pw" className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">
						Senha
					</label>
					<input
						id="invite-pw"
				...
				<div>
					<label htmlFor="invite-pw2" className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">
						Confirmar senha
					</label>
					<input
						id="invite-pw2"
